...
app.get(
  ...
  async (request, response) => {
    ...
    const getFollowersIdsQuery = `SELECT following_user_id FROM follower WHERE follower_user_id= ${getUserId.user_id};`;
    const getFollowersIdsArray = await database.all(getFollowersIdsQuery);
    //console.log(getFollowersIdsArray);
    const getFollowersIdsSimple = getFollowersIdsArray.map((eachUser) => {
      return eachUser.following_user_id;
    });
    //query
    const getTweetQuery = `SELECT user.username, tweet.tweet, tweet.date_time AS dateTime 
      FROM user INNER JOIN tweet 
      ON user.user_id= tweet.user_id WHERE user.user_id IN (${getFollowersIdsSimple})
...
app.get("/user/followers/", authenticateToken, async (request, response) => {
  ...
  const getUsersQuery = `
        ...
        WHERE follower.follower_user_id LIKE ${search_q};`;
  const userArray = await database.all(getUsersQuery);
  response.send(userArray);
});

app.get("/tweets/:tweetId/", authenticateToken, async (request, response) => {
  const { tweetId } = request.params;
  const getUser = `SELECT * FROM tweet
    WHERE tweet_id = ${tweetId};`;
  const userArray = await database.get(getUser);
  response.send(userArray);
});
...
app.delete(
  ...
  async (request, response) => {
    ...
    WHERE
        tweet_id = ${tweetId};`;
    const deleteQueryData = await database.run(deleteQuery);
    response.send("Tweet Removed");
  }
);
